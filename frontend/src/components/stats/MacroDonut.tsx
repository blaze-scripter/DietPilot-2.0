    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 24 }}>
      ...
      <div style={{ position: 'relative', width: 120, height: 120, flexShrink: 0 }}>
        <svg viewBox="0 0 130 130" style={{ width: '100%', height: '100%', transform: 'rotate(-90deg)' }}>
          {total === 0 && (
            <circle cx="65" cy="65" r={r} fill="none" stroke="var(--tb-input-bg)" strokeWidth="14" strokeLinecap="round" />
          )}
          {total > 0 && (
            <>
        ...
        <div style={{
          ...
          <span style={{ fontSize: '1rem', fontWeight: 800, color: 'var(--tb-text)', fontFamily: 'var(--font-display)' }}>
            {Math.round(total)}g
          </span>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, flex: 1 }}>
        {legend.map((item) => (
          <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div>
              <p style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--tb-text)', fontFamily: 'var(--font-display)' }}>{item.label}</p>
              <p style={{ fontSize: '0.625rem', fontWeight: 500, color: 'var(--tb-text-secondary)' }}>{Math.round(item.value)}g · {item.pct}%</p>
            </div>
          </div>
        ))}
      </div>
